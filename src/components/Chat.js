...
export default function Chat({ car, expenses, serviceRecords, reminders, currency, onAddExpense, onRequireAuth }) {
  ...
  const getFullContext = () => {
    ...
    if (expenses.length > 0) {
      ...
      const expList = sorted.map((e) => {
        let line = `${new Date(e.date).toLocaleDateString('ru-RU')}: ${e.description} — ${e.amount}${e.liters ? `, ${e.liters}л` : ''}${e.mileage ? `, ${e.mileage}км` : ''}`
        if (e.notes) line += ` [заметка: ${e.notes}]`
        return line
      }).join('\n')
      const total = expenses.reduce((s, e) => s + e.amount, 0)
      const byCat = {}
      expenses.forEach((e) => { byCat[CATEGORIES[e.category]?.label || e.category] = (byCat[CATEGORIES[e.category]?.label || e.category] || 0) + e.amount })
      const catStr = Object.entries(byCat).map(([k, v]) => `${k}: ${v}`).join(', ')
      parts.push(`РАСХОДЫ (${expenses.length} записей, всего ${total}):\nПо категориям: ${catStr}\nПоследние:\n${expList}`)
    }

    // Service records
    if (serviceRecords && serviceRecords.length > 0) {
      const srvList = serviceRecords.slice(0, 15).map((r) => {
        let line = `${new Date(r.date).toLocaleDateString('ru-RU')}: ${r.description}${r.cost ? ` — ${r.cost}` : ''}${r.mileage ? `, ${r.mileage}км` : ''}`
        if (r.notes) line += ` [заметка: ${r.notes}]`
        return line
      }).join('\n')
      parts.push(`СЕРВИСНЫЕ ЗАПИСИ (${serviceRecords.length}):\n${srvList}`)
    }

    // Reminders
    if (reminders && reminders.length > 0) {
      const remList = reminders.map((r) => {
        return `${r.label}: последнее ${r.lastDate ? new Date(r.lastDate).toLocaleDateString('ru-RU') : '?'}${r.intervalKm ? `, каждые ${r.intervalKm}км` : ''}${r.intervalDays ? `, каждые ${r.intervalDays}дн` : ''}`
      }).join('\n')
      parts.push(`НАПОМИНАНИЯ (${reminders.length}):\n${remList}`)
    }
    ...
    try {
      ...
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          image: imageData,
          car,
          expensesSummary: getFullContext(),
        }),
      })
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 pt-4 pb-3 border-b border-border shrink-0 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-[15px] font-semibold text-dark">CarNote AI</p>
      ...
      {/* Input */}
      <div className="px-4 pb-[max(env(safe-area-inset-bottom,8px),8px)] pt-2 border-t border-border shrink-0">
