...
export default function Dashboard({ car, cars, expenses, currency, onAddExpense, onEditExpense, onDeleteExpense, onOpenProfile, onOpenStats, onSwitchCar, onRequireAuth }) {
  ...
  const recent = showAllExpenses ? filtered : filtered.slice(0, 15)
    ...
    <div className="bg-white pb-4">
      ...
      <div className="px-5 pt-5 pb-4 flex items-start justify-between shrink-0">
        ...
        <div className="flex items-center gap-2 mt-1">
          <button onClick={() => { if (onRequireAuth && !onRequireAuth()) return; setEditExpense(null); setShowAdd(true) }} className="w-9 h-9 bg-dark rounded-xl flex items-center justify-center text-gold">
            <IconPlus />
          </button>
      ...
      {(expenses.length > 0) ? (
        <div className="px-5 mb-5">
          <div className="flex items-center justify-between mb-2.5">
            <p className="text-xs font-medium text-muted tracking-wider">
              {filterCat !== 'all' ? `${CATEGORIES[filterCat]?.icon} ${CATEGORIES[filterCat]?.label}` : 'ЗАПИСИ'}
              {filterCat !== 'all' && ` (${filtered.length})`}
            </p>
            <button onClick={() => setShowFilters(!showFilters)}
              className={`text-xs font-medium px-2.5 py-1 rounded-lg transition-colors ${showFilters || filterCat !== 'all' ? 'bg-gold-50 text-gold-dark' : 'text-subtle'}`}>
              ⚙ Фильтр
            </button>
          </div>
            ...
            <div className="mb-3 fade-in">
              ...
              <div className="flex gap-1.5 overflow-x-auto pb-2 -mx-1 px-1 no-scrollbar">
                <button onClick={() => setFilterCat('all')}
                  className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${filterCat === 'all' ? 'bg-dark text-white' : 'bg-surface border border-border text-subtle'}`}>
                  Все
                </button>
                {Object.entries(CATEGORIES).map(([id, cat]) => (
                  <button key={id} onClick={() => setFilterCat(id)}
                    className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${filterCat === id ? 'bg-dark text-white' : 'bg-surface border border-border text-subtle'}`}>
                    {cat.icon} {cat.label}
                  </button>
                ))}
              </div>
              ...
              <div className="flex gap-2 mt-2">
                <button onClick={() => setSortBy('date')}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium ${sortBy === 'date' ? 'bg-gold-50 text-gold-dark border border-gold' : 'bg-surface border border-border text-subtle'}`}>
                  По дате
                </button>
                <button onClick={() => setSortBy('amount')}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium ${sortBy === 'amount' ? 'bg-gold-50 text-gold-dark border border-gold' : 'bg-surface border border-border text-subtle'}`}>
                  По сумме
                </button>
                {filterCat !== 'all' && (
                  <button onClick={() => { setFilterCat('all'); setSortBy('date') }}
                    className="px-3 py-1.5 rounded-lg text-xs font-medium text-red-400 bg-red-50 border border-red-200">
                    ✕ Сброс
                  </button>
                )}
              </div>
            ...
            return (
              <div key={exp.id}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {exp.notes && <p className="text-xs text-subtle mt-0.5 line-clamp-1">💬 {exp.notes}</p>}
                </div>
                <p className="text-[14px] font-semibold text-dark shrink-0">{fmtCurrency(exp.amount, cur)}</p>
              </div>
            )
          }) : (
            <p className="text-center text-sm text-muted py-6">Нет записей{filterCat !== 'all' ? ' в этой категории' : ''}</p>
          )}
          ...
          {filtered.length > 15 && (
            <button onClick={() => setShowAllExpenses(!showAllExpenses)}
              className="w-full text-center text-sm text-gold-dark font-semibold mt-3 py-2">
              {showAllExpenses ? `Скрыть (показано ${filtered.length})` : `Все записи (${filtered.length})`}
            </button>
          )}
        </div>
      ) : (
        <button onClick={() => { if (onRequireAuth && !onRequireAuth()) return; setShowAdd(true) }} className="mx-5 w-[calc(100%-40px)] text-center py-16 rounded-2xl border-2 border-dashed border-border active:bg-surface transition-colors mb-5">
        ...
        <div className="fixed inset-0 z-[60] bg-dark/40 fade-in flex items-end justify-center" onClick={() => setActionExpense(null)}>
          <div className="w-full max-w-[430px] bg-white rounded-t-3xl slide-up" onClick={(e) => e.stopPropagation()}>
            <div className="p-5 pb-8">
              ...
              <button onClick={() => { if (onRequireAuth && !onRequireAuth()) return; setEditExpense(actionExpense); setShowAdd(true); setActionExpense(null) }}
      ...
      {showAdd && <AddExpense editData={editExpense} currency={currency} onAdd={editExpense ? onEditExpense : onAddExpense} onClose={() => { setShowAdd(false); setEditExpense(null) }} />}
    </div>
  )
}
