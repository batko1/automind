    ...
    <div className="fixed inset-0 flex flex-col justify-center px-7 bg-white overflow-hidden fade-in">
      <div className="text-center mb-5">
        <div className="inline-flex items-center gap-2 bg-dark px-4 py-1.5 rounded-full">
          ...
          <span className="text-xs font-semibold text-gold tracking-[0.15em]">CARNOTE</span>
        </div>
      </div>
      ...
      {onSignIn && (
        <button onClick={onSignIn} className="w-full py-4 mt-3 rounded-2xl border-2 border-gold text-gold-dark font-semibold text-base text-center">
          Войти в аккаунт
        </button>
      )}
    </div>
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {driveTypes.map((d) => (
              <button key={d.id} onClick={() => setCar({ ...car, drivetrain: d.id })}
                className={`py-2.5 rounded-xl text-xs font-semibold transition-all text-center ${car.drivetrain === d.id ? 'bg-gold-50 border border-gold text-dark' : 'bg-surface border border-border text-subtle'}`}>
                {d.label}
              </button>
            ))}
          </div>
...
export default function Onboarding({ onComplete, onSignIn }) {
  ...
  if (step === 0) return <WelcomeScreen onNext={() => setStep(1)} onSignIn={onSignIn} />
