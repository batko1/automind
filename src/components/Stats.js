...
export default function Stats({ car, expenses, currency }) {
  ...
  // Fuel consumption calculation (л/100км)
  const { fuelConsumption, avgConsumption } = useMemo(() => {
    ...
    for (let i = 1; i < fuelWithMileage.length; i++) {
      const prev = fuelWithMileage[i - 1]
      const curr = fuelWithMileage[i]
      const km = (curr.mileage || 0) - (prev.mileage || 0)
      if (km > 0 && km < 3000) {
        const consumption = (curr.liters / km) * 100
        if (consumption > 2 && consumption < 50) {
          points.push({
            date: new Date(curr.date).toLocaleDateString('ru-RU', { day: '2-digit', month: '2-digit' }),
    ...
    const avg = points.length > 0 ? points.reduce((s, p) => s + p.consumption, 0) / points.length : 0
    return { fuelConsumption: points, avgConsumption: avg }
  }, [expenses])
    ...
    <div className="bg-white pb-4">
      ...
      <div className="px-5 pt-5 pb-4">
        <div className="flex items-center justify-between">
          ...
          <div className="text-xs text-muted bg-surface px-3 py-1.5 rounded-lg border border-border">
            Нажмите «Главная» для возврата
          </div>
        </div>
      </div>
        ...
        <div className="px-5 mb-5">
          ...
          <div className="grid grid-cols-3 gap-2.5">
            ...
            <div className="bg-surface rounded-2xl border border-border p-3.5 text-center">
              ...
              <p className="text-[10px] text-muted font-medium mt-0.5">{cur === 'RUB' ? '₽' : cur} / ЛИТР</p>
            </div>
            <div className="bg-surface rounded-2xl border border-border p-3.5 text-center">
          ...
          {/* Consumption chart (л/100км) */}
          {fuelConsumption.length > 1 && (
            <div className="mt-4 bg-surface rounded-2xl border border-border p-4">
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs font-medium text-dark">Расход топлива</p>
                <p className="text-xs text-muted">л / 100 км</p>
              </div>
              <div style={{ height: 120 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={fuelConsumption}>
                    ...
                    <XAxis dataKey="date" tick={{ fontSize: 10, fill: '#b0b0be' }} axisLine={false} tickLine={false} />
                    <Tooltip
                      contentStyle={{ background: '#1a1a2e', border: 'none', borderRadius: 12, fontSize: 12, color: '#fff' }}
                      formatter={(v) => [`${v.toFixed(1)} л/100км`, 'Расход']}
                      labelStyle={{ color: '#C8A45C' }}
                    />
                    <Area type="monotone" dataKey="consumption" stroke="#C8A45C" strokeWidth={2} fill="url(#fuelGrad)" dot={{ fill: '#C8A45C', r: 3 }} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
              <div className="flex justify-between mt-2">
                <p className="text-[11px] text-subtle">Средний: <span className="font-semibold text-dark">{avgConsumption.toFixed(1)}</span> л/100км</p>
                {fuelConsumption.length > 0 && (
                  <p className="text-[11px] text-subtle">Последний: <span className="font-semibold text-dark">{fuelConsumption[fuelConsumption.length - 1].consumption.toFixed(1)}</span></p>
                )}
              </div>
            </div>
          )}
